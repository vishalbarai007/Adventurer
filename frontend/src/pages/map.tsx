...
const Map: React.FC = () => {
		...
		if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(
				...
				(error) => {
					...
					setError(
						"Could not access your location. Using approximate location from IP.",
					);
				},
			);
		} else {
			...
			fetchNearbySpots(null);
			setError(
				"Your browser doesn't support geolocation. Using approximate location from IP.",
			);
		}
	};
	...
	const fetchNearbySpots = async (
		...
		try {
			console.log("Fetching nearby spots with coordinates:", coordinates);
			...
			setError(
				"Failed to fetch nearby tourist spots. Please ensure the backend server is running.",
			);
			setLoading(false);
		}
	};
	...
	const userIcon = L.icon({
		...
		iconAnchor: [12, 41],
		className:
			"text-blue-500 bg-white rounded-full border-2 border-blue-500",
	});
				...
				<div className="w-1/3 bg-white shadow-inner overflow-y-auto">
					<div className="p-4">
						<h2 className="text-xl font-semibold mb-4">
							Tourist Spots ({spots.length})
						</h2>

						{spots.length === 0 && !loading ? (
							<p className="text-gray-500">
								No tourist spots found nearby
							</p>
						) : (
